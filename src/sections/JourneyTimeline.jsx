...
export default function JourneyTimeline() {
  ...
  return (
    <section id="journey" className="py-24 bg-[#040d0a] border-t border-slate-900 font-mono-tech relative select-text">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16 space-y-3">
          ...
          <p className="text-slate-400 text-xs sm:text-sm font-sans">
            From high school computer fundamentals to SRM IST CS Engineering B.Tech & ResQNet disaster systems.
          </p>
        </div>
        ...
        <div className="max-w-4xl mx-auto space-y-4">
          {journeyTimelineData.map((item) => {
            ...
            return (
              <div
                key={item.year}
                className={`os-glass-panel rounded-2xl border transition-all overflow-hidden ${
                  isExpanded ? 'border-emerald-500/40 shadow-2xl bg-slate-900/70' : 'border-slate-800 hover:border-slate-700'
                }`}
              >
                {/* Accordion Header */}
                <button
                ...
                >
                  <div className="flex items-center space-x-4">
                    <span className="text-2xl font-extrabold font-sans text-emerald-400 bg-emerald-950/80 px-3 py-1 rounded-xl border border-emerald-500/30">
                      {item.year}
                    </span>
                    <div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {item.skillsAcquired.map((skill) => (
                          <span key={skill} className="text-xs bg-slate-900 text-emerald-300 px-3 py-1 rounded-lg border border-emerald-500/20 font-mono-tech">
                            {skill}
                          </span>
                        ))}
                      </div>
