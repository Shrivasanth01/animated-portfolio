...
export default function ArchitectureDiagram({ nodes }) {
  ...
  return (
    <div className="w-full glass-panel rounded-xl p-6 border border-cyan-500/20 shadow-xl overflow-hidden font-mono-tech">
            ...
            <div key={node.id} className="flex items-center flex-1 relative">
              ...
              >
                <div
                  className={`w-8 h-8 rounded-md flex items-center justify-center mb-2 transition-colors ${
                    isActive ? 'bg-cyan-500 text-slate-950 shadow-md' : 'bg-slate-800 text-slate-400'
                  }`}
                >
              ...
              {index < nodes.length - 1 && (
                <div className="flex-1 h-0.5 bg-slate-800 mx-1 relative overflow-hidden">
                  <div
                    className={`absolute top-0 bottom-0 w-8 bg-gradient-to-r from-transparent via-cyan-400 to-transparent transition-all duration-700 ${
          ...
          return (
            ...
            >
              <div className="flex items-center space-x-3">
                <div className={`p-2 rounded-md ${isActive ? 'bg-cyan-500 text-slate-950' : 'bg-slate-800 text-slate-400'}`}>
                  <IconComponent className="w-4 h-4" />
                </div>
                <div>
              ...
              {isActive && <span className="text-[10px] text-cyan-400 font-bold">PROCESSING ›</span>}
            </div>
          );
        })}
      </div>
